...
ssize_t safe_read(int fd, void *buf, size_t count) {  // Alterado para size_t
    size_t off = 0;
    while (off < count) {
...
ssize_t safe_write(int fd, const void *buf, size_t count) {  // Alterado para size_t
    size_t off = 0;
    while (off < count) {
...
/* ---------------- alocação verificada ---------------------------------- */
void *xmalloc(size_t n) {  // Alterado para size_t
    void *p = malloc(n);
    if (!p) die("malloc falhou (%zu bytes)", n);
    return p;
}

void *xcalloc(size_t nm, size_t sz) {  // Alterado para size_t
    void *p = calloc(nm, sz);
    if (!p) die("calloc falhou (%zu×%zu)", nm, sz);
    return p;
}

/* ---------------- cronômetro monotônico -------------------------------- */
